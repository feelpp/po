...
\mathbf{g_i}\cdot \mathbf{n}\restr = 0\\
\rot \mathbf{g_i}\cdot \mathbf{n}\restr = 0\\
(\rott \mathbf{g_i}\cdot \mathbf{n}\restr = 0) \end{aligned}$} ;
\node[draw,scale=\taille,fill=yellow!50] (lambdagi) at (10, 5.5) {$(\Lambda_i,\mathbf{g_i})$} ;
...
\sum_{i=1}^\infty\frac{\partial c_i}{\partial t}\bm{g_i} &+ \sum_{i=1}^\infty\sum_{j=1}^\infty c_i\lambda_ic_j(\bm{g_i}\times \bm{g_j}) + \sum_{i=1}^\infty c_i\lambda_i(\bm{g_i}\times \mathbf{a})\\
& + \sum_{i=1}^\infty c_i((\rot\mathbf{a})\times \mathbf{g_i}) + \grad \pi_{\mathbf{a}} +\frac{1}{Re}\sum_{i=1}^\infty c_i\lambda_i^2\mathbf{g_i} - \mathbf{h_a} = 0 \end{aligned}$};
...
% de la même manière que pour $\mathbf{b}$, voir \ref{rkb}, on a $\mathbf{u}\in D^1$ et donc $\rott\mathbf{u}\cdot\mathbf{n}\restr = 0 \neq \alpha_2$.
...
\label{pression}
\todo[inline]{intro + $\alpha_2 <=> p$, choix stage}
