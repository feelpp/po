...
curl(curl(v))=\rott v
\end{eqnarray*}
\todo[inline]{curl est anglais, rot est francais, il faudrait choisir
probablement}
\todo[inline]{serait il possible de mettre les champs vectoriels en gras, et les
tenseurs de rang 2(matrices) avec une barre au dessus ? ca rendrait la lecture
plus aisée}
...
\end{tabular}
\end{center}
\todo[inline]{ces tableaux sont intéressants, il faudrait les commenter un petit
peu}
...
calculer $\grad\psi_0$.
\todo[inline]{ÇP: Certes, ce pose tout de même la résolution de (\ref{psi0}), quelle
stratégie adopter ?on peut fixer la constante de différentes manières:
multiplicateur de Lagrange, penalisation, algébrique(solveur itératif avec
initial guess qui n'est pas dans le noyaux.)}
...
retrouver $a$.
\todo[inline]{il faudra discuter les différentes alternatives de résolution
ici. soit on résout le système couplé (\ref{curlb}) ou le système découplé en (\ref{psi0})+~(\ref{curlb})}
...
carré\todo{attention il faudrait mieux écrire $\rott$ que \emph{au carré}}.
...
\label{org}
\end{figure}
\todo[inline]{CP: excellent graphique!}
