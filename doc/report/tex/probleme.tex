...
\todo[inline]{espace [], bm mathbf, problème amsthm, hdiv, hrot, mathrm}
...
\todo[inline]{cdt non standard, peu étudié + ref( éléctro, Girault, Perrier), intérêt PO : séparation des variables => gain de temps, suppr loi de paroi}
...
\label{local}
\todo[inline]{précision existence unicité}
...
Il reste encore plusieurs questions concernant la manière de relever $\alpha_1$. En effet, écris de cette manière, $\mathbf{b}\in D^1(\Omega)$ et donc si $\mathbf{b}\cdot\mathbf{n}=0$ alors $\rott\mathbf{b}\cdot\mathbf{n}=0$ aussi. Ce problème est donc encore ouvert.\\
...
%% Comme les conditions aux bords sont respectées par tous les $\mathbf{g_i}$, $\mathbf{u}$, en tant que combinaison linéaire de ceux-ci, les respectent aussi.\\
...
%% Comme on peut le voir dans la formulation, $\alpha_1$ n'apparaît pas, de plus, comme expliqué précédemment, $\mathbf{b}$ appartient à $D^1$, et ne peut donc pas satisfaire la condition $\rott\mathbf{b}\cdot \mathbf{n}$ :
%% \[ \rott\mathbf{b}\cdot \mathbf{n} = \rott\left(\sum h_i \mathbf{g_i}\right)\cdot\mathbf{n} = \sum h_i \rott \mathbf{g_i}\cdot\mathbf{n} = \sum h_i\lambda^2 \underbrace{\mathbf{g_i}\cdot\mathbf{n}}_{=0} = 0 \]
%% il faudrait donc chercher un autre système pour relever la condition $\rot\mathbf{a}\cdot\mathbf{n}=\alpha_1$.
...
Or, $\bm{\varphi}\in D^1(\Omega)$ donc $\div\bm{\varphi}=0$ et $\bm{\varphi}\cdot \mathbf{n}=0$ sur $\partial\Omega$, le terme de pression s'annule donc sous cette forme.\\
...
%label={[xshift=-1.5cm,yshift=-0.1cm](\ref{discbd1})}
] (pbb) at (8,-7.5) 
{$
