...
Comme on cherche les solutions dans $[H^1(\Omega)]^3$, on utilise des éléments de Lagrange et non de Nedelec pour approcher les $\mathbf{g_i}$. Pour le terme de pression fictif, on utilise aussi des éléments de Lagrange, scalaire et d'ordre 1.\\ 
...
\begin{figure}[H]
	...
	\label{resultats}
\end{figure}
\todo[inline]{explication modes corrects :\\
lien avec laplacien, les erreurs proviennent de l'implémentation de l'opérateur curl}
Cependant, dû au manque de régularité de $\rot\mathbf{g_i}$, le calcul du rotationnel induit des erreurs. Ainsi si on calcul l'erreur entre le rotationnel du champ et le champ multiplier par sa valeur propre, ou même lors du calcul de la condition $\rot\mathbf{g_i}\cdot\mathbf{n}$, on n'obtient pas les résultats escomptés :
\begin{center}
