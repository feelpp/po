...
public :
    ...
    void init()
	    ...
	    std::cout<<"Trying run "<<M_acusimRunId<<"\n";
	    bool success = adbOpenRun(M_adbHd,M_acusimRunId);
...
/*
  ...
  */
    ...
    void run(bool doMeshConversion);
...
template <typename MeshType>
void
ConverterAcusimDatabase<MeshType>::run(bool doMeshConversion)
{
    this->init();
    ...
    if ( doMeshConversion )
    {
        ...
        return ;
         ...
         M_feelppDatabase.setMeshFilename((boost::format("mesh_p%1%.json")%this->worldComm().size() ).str());
    ...
    auto mesh = M_feelppDatabase.loadMesh( MESH_NO_UPDATE_MEASURES|MESH_GEOMAP_NOT_CACHED );
    auto mem2  = Environment::logMemoryUsage("memory usage after update for use");
    std::cout << "[Mesh::updateForUse] resident memory after load mesh: " << mem2.memory_usage/1.e9  << "GBytes\n";
...
ConverterAcusimDatabase<MeshType>::loadMesh()
{
    ...
    M_nodeIds = new int[M_nNodes];
    success = adbGetInts0( M_adbHd,(char*)"usrIds", M_nodeIds, M_nNodes , 1);
        CHECK( success ) << "fail to get user ids\n";
    for ( int k=0;k<M_nNodes;++k )
        M_nodeIdToContainerId[ M_nodeIds[k] ] = k;
#endif
    mesh_ptrtype mesh;
    ...
    auto *crd= new double[3*M_nNodes];
    //crd = (double*)malloc(3*M_nNodes*sizeof(double));
