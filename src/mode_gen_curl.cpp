...
EigenProblem::run()
{
    ...
    if ( !modes.empty() )
    {
        LOG(INFO) << "nev " << nev << ", ncv " << ncv << ", proc " << Environment::numberOfProcessors() << ", time " << t.elapsed() << std::endl;
        ...
        for( auto const& mode : modes )
        {
            ...
            //a2 = form2(Vh,Vh);
            //a2 = integrate(elements(mesh), divt(g0[i])*div(vv) + gradt(g0[i])*grad(vv));
            //l2 = integrate(elements( emsh), gradv(g[i])*grad(vv));
            //a2+= on(boundaryelements(mesj), g0[i], cst(0.) );
            //a2.solve
