...
*/
   ...
   *****************************/
   ...
   double resPrim = 10e-6;
   double resDual = 10e-4 ;
...
*/
   ...
   Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> Rho(nEigenValues,1);
   Rho.setOnes();
   Rho = rho * Rho;
 
   double a = gamma / rho ;
   Eigen::Matrix<std::complex<double>,Eigen::Dynamic,1> u(nEigenValues);
   Eigen::Matrix<std::complex<double>,Eigen::Dynamic,1> v(nEigenValues);
   Eigen::Matrix<std::complex<double>,Eigen::Dynamic,1> v0(nEigenValues);
   v0.setZero();
   ...
   while ((resPrim > epsPrim) && (resDual>epsDual) )
   {
	   ...
	   u = beta -1/rho*lambda ;
           alpha = (Prho.inverse()) * (q+0.5*rho*u);

           //auto x = Plow.colPivHouseholderQr().solve(Rho+0.5*u);
           //alphaN = Plow.adjoint().colPivHouseholderQr().solve(x);

           // beta minimization step with soft tresholding of v
           v = alpha+1/rho*lambda ;
           betaN = (v.array().abs()>a).select((v.array()-v.array().sign()*a),v0);

           // lagrange multiplier update step
           lambda = lambda+rho*(alpha-betaN);

           // Primal and dual residuals
           resPrim = (alpha-betaN).norm();
           resDual = rho*(betaN-beta).norm();

           // Stopping criteria
           epsPrim = std::sqrt((double) nEigenValues) * epsAbs + epsRel * std::max(alpha.norm(),betaN.norm());
           epsDual = std::sqrt((double) nEigenValues) * epsAbs + epsRel * lambda.norm() ;

           if ( myDb.worldComm().isMasterRank() )
              std::cout<<"AMM Step "<<ADMMstep<<": resPrim "<< resPrim << " / epsPrim "<<epsPrim<<"; resDual "<< resDual<<" / epsDual "<<epsDual<<"\n";


   }
   if ( myDb.worldComm().isMasterRank() )
      std::cout<<"alpha result :\n"<<alpha<<"\n";
    ...
    for ( int i=0;i<nModes;++i )
    {
	 ...
	 for ( int j=0;j<nTimeStep;++j )
		...
		PHI_imag = PHI_imag + a.imag()*uj;
         ...
         // Reconstruction
         uRec = uRec + (alpha(i)*std::pow(eigenValues_dmd(nEigenValues -1 - i),(nTimeStep-1)*dt)).real()*PHI_real \
                     - (alpha(i)*std::pow(eigenValues_dmd(nEigenValues -1 - i),(nTimeStep-1)*dt)).imag()*PHI_imag ; 
...
//        myDb.save(timeSetIndex[i],"PHI_imag",PHI_imag );
	 ...
	 e->step( i )-> add( "PHI_imag", PHI_imag );
         ...
         if (i==nTimeStep-2)
         {
            e->step( i )->add("Reconstruction",uRec);
